...
def draw_landmark_text(frame, thk=2, fontsz=0.5, color=(0, 0, 255)):
    frame = frame.copy()
    cv2.putText(frame, "Alinhe o rosto", (60, 20), 0, fontsz * IMG_SIZE / 255, color, thk)
    cv2.putText(frame, "X - Para Iniciar", (60, 245), 0, fontsz * IMG_SIZE / 255, color, thk)
    return frame


def draw_calib_text(frame, thk=2, fontsz=0.5, color=(0, 0, 255)):
    frame = frame.copy()
    cv2.putText(frame, "Alinhe o rosto", (40, 20), 0, fontsz * IMG_SIZE / 255, color, thk)
    cv2.putText(frame, "W -> + zoom", (60, 40), 0, fontsz * IMG_SIZE / 255, color, thk)
    cv2.putText(frame, "S -> - zoom", (60, 60), 0, fontsz * IMG_SIZE / 255, color, thk)
    cv2.putText(frame, "X -> Iniciar", (60, 245), 0, fontsz * IMG_SIZE / 255, color, thk)
    return frame
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            if cv2.getWindowProperty('Webcam', cv2.WND_PROP_VISIBLE) < 1.0:
                break
            elif is_calibrated and cv2.getWindowProperty('Fake', cv2.WND_PROP_VISIBLE) < 1.0:
                break
            ...
            if key == 27: # ESC
                ...
                if not is_calibrated:
                    cv2.namedWindow('Fake', cv2.WINDOW_GUI_NORMAL)
                    cv2.moveWindow('Fake', 600, 250)
            ...
            cv2.imshow('Webcam', preview_frame[..., ::-1])
